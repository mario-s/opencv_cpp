...
class RANSACparams {
    ...
    public:
        //constructor
        RANSACparams(int _iter, int _min_inliers, float _dist_thresh, int _N) {
            iter = _iter;
...
class ellipseFinder {
    private:
        ...
        // input image
        // contours in image
        // Matrix representing conic section of detected ellipse // function to fit ellipse to a contour
        vector<vector<Point> > contours;
        Mat Q;
        Mat fit_ellipse(vector<Point>);
        ...
        vector<float> distance(Mat, vector<Point>);
        float distance(Mat, Point);
        void draw_ellipse(Mat);
        vector<Point> ellipse_contour(Mat);

        void draw_inliers(Mat, vector<Point>);
        // RANSAC parameters
...
void ellipseFinder::draw_ellipse(Mat Q) {
    ...
    rectangle(img_show, rect, Scalar(255, 0, 255), 3);
    circle(img_show, center, radius, Scalar(255, 0, 0), 3);
    Point2f vertices[4]; r_rect.points(vertices);
    
    for(int i = 0; i < 4; i++)
        line(img_show, vertices[i], vertices[(i + 1) % 4], Scalar(0, 255, 0), 2); //green
...
void ellipseFinder::draw_inliers(Mat Q, vector<Point> c) {
   ...
   drawContours(img_show, contours, -1, Scalar(0, 0, 255), 3);
